...
export async function sendOtpEmail(email: string, otp: string): Promise<boolean> {
  ...
  const htmlContent = `
    <div style="background-color: #0A0A0A; color: #EAEAEA; font-family: monospace; padding: 40px; border: 1px solid #2A2A2A; max-width: 600px; margin: 0 auto;">
      ...
      <p style="font-size: 14px; line-height: 1.6;">You have requested security access to the Nyxel Technologies Vault.</p>
      ...
      <p style="font-size: 10px; color: #666666; text-align: center; margin-bottom: 0;">Built in stealth. Deployed at scale. Nyxel Technologies Collective.</p>
    </div>
  `;
